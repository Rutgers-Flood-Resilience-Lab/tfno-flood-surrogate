...
├── data/                — empty directory skeleton matching the expected layout (see below)
└── examples/            — example inference output (see Example output, below)
```
...
```
![Static features preview](data/parms_bands/preview/static_features.png)
*(Barbados domain, 936×1080 @ ~30m — the actual domain this package's example checkpoint/output below was trained and run on. Ocean masked out on the land-only fields. Individual full-size panels are also in [data/parms_bands/preview/](data/parms_bands/preview/).)*
...
[examples/sample181_max_depth.png](examples/sample181_max_depth.png) — temporal-max water depth (the per-pixel maximum reached over the whole rollout), ground truth vs. predicted vs. error, produced by `inference.py`'s `--eval` output:

![Example max-depth output](examples/sample181_max_depth.png)

[examples/sample181_rollout.gif](examples/sample181_rollout.gif) — the full hour-by-hour rollout (rainfall / prediction / ground truth / error), produced by `inference.py`'s `--gif` output.

**Note on provenance**: both files were generated by a **hybrid TFNO+CNN fusion variant** (a per-layer spectral+convolutional fusion built on top of the same `TFNO` backbone, from outside this package's scope — see this repo's README intro), not by the plain `TFNOFlood` model included here. They're included as a representative example of this line of work's inference-output format and visual quality; `inference.py` in this package produces the exact same figure/GIF layout (identical code path), just from a `TFNOFlood` checkpoint rather than the hybrid one. Regenerate a `TFNOFlood`-specific example by running `inference.py --model_type fno --eval --gif` against a checkpoint trained with `train_fno.py`.
